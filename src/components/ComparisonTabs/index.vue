...
const handle = () => {
  console.log('handle', props.comparisonTabs)
}
...
<template>
  <div class="card">
    <el-tabs v-model="activeName" class="h150 flex-1">
              ...
              <el-col v-for="(card, idx) in tab.formItems" :key="idx" :span="card.span || 3">
                ...
                <el-form-item v-else :label="card.title">
                  <component :is="`el-${card.type ?? 'select'}`" v-bind="card.attrs" v-model="card.value">
                    <el-option
                      v-for="(option, ix) in card.options" :key="ix" :label="option.label"
                      :value="option.value"
                    />
                  </component>
          ...
          <div v-if="goodsInfo" class="goodsInfo">
            ...
            <div class="infos">
              ...
              <div class="rate fc gap20">
                ...
                <el-rate :model-value="4" disabled show-score score-template="{value}.0" />
                <i text="#666">529条评价</i>
      ...
      <el-tab-pane label="商品ASIN" name="2">
        22
      </el-tab-pane>
      <el-tab-pane label="商品 VS 店铺" name="3">
        33
      </el-tab-pane>
      <el-tab-pane label="商品 VS 行业" name="4">
        444
      </el-tab-pane>
    </el-tabs>
  </div>
