...
const handle_tab_activeName = async (val: any) => {
  ...
  evaluatePieBarChart_data.value = await get_original_sound_insights(statList.value, [])
};
...
const handle_tab_evaluatePieBarChart_select = async (val: any) => {
  ...
  const data = await get_system_insights_parent_label(statList.value, [], Number(val.currentIndex) + 1, val.selected_value)
...
const get_original_sound_insights = async (v, a) => {
  const original_sound_insights = await http.get(`/system/original_sound_insights`, {
...
const get_system_insights_parent_label = async (v, a, current_level, label_id) => {
...
const handletab = async (index) => {
  ...
  if (index == 0) {
    await get_customer_feedback_analysis(detailList.value, cate_hierarchy_data.value);
  } else {
  }
};
